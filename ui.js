...
const UI = (() => {
  ...
  function renderNav(activePage) {
    const items = GEOWIRE.navigationItems.map(item => {
      ...
      return `<a href="${_escHtml(item.href)}" class="nav-link ${isActive ? 'nav-link-active' : ''}" aria-current="${isActive ? 'page' : 'false'}">${_escHtml(item.label)}</a>`;
    }).join('');
  ...
  function renderFooter() {
    ...
    const secondaryLinks = (GEOWIRE.secondaryNavItems || []).map(n=>`<a href="${_escHtml(n.href)}">${_escHtml(n.label)}</a>`).join('');
    return `<footer class="site-footer" role="contentinfo">
      <div class="footer-inner container">
        ...
        ${secondaryLinks ? `<div class="footer-secondary-links" style="margin-top:8px;display:flex;flex-wrap:wrap;gap:8px 16px;">${secondaryLinks.replace(/<a /g,'<a style="font-size:12px;color:var(--gw-text-secondary,#94A3B8);text-decoration:none;" ')}</div>` : ''}
        <div class="footer-legal">
          ...
          <p class="footer-disclaimer">GeoWire is an independent recession intelligence platform. Seed data is editorial/demo content unless marked LIVE. Not financial or political advice.</p>
        </div>
      </div>
    </footer>`;
  }
